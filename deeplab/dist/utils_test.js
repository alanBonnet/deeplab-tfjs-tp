...
describe('Utilities', function () {
    it('Colormaps and labels must match in length.', function () {
        var bases = ['pascal', 'cityscapes', 'ade20k'];
        var colormapLengths = bases.map(function (base) {
            return utils_1.getColormap(base).length;
        });
        // Make an exception for pascal, which has a palette larger than
        // the number of labels
        var labelsLengths = bases.map(function (base) {
            return base === 'pascal' ? config_1.config['DATASET_MAX_ENTRIES']['PASCAL'] :
                utils_1.getLabels(base).length;
        });
        expect(colormapLengths).toEqual(labelsLengths);
    });
    it('The PASCAL colormap coincides with the original.', function () {
        expect(utils_1.createPascalColormap()).toEqual([
